...
def friendship_encoder(graph: nx.Graph, name_dict: dict[int, str]) -> str:
    ...
    nodes_string = create_node_string(name_dict, len(graph.nodes()))
    output = "G describes a friendship graph among nodes %s.\n" % nodes_string.strip()
    if graph.edges():
...
def coauthorship_encoder(graph: nx.Graph, name_dict: dict[int, str]) -> str:
    ...
    nodes_string = create_node_string(name_dict, len(graph.nodes()))
    output = "G describes a coauthorship graph among nodes %s.\n" % nodes_string.strip()
    if graph.edges():
...
def social_network_encoder(graph: nx.Graph, name_dict: dict[int, str]) -> str:
    ...
    if graph.is_directed():
        raise ValueError("Social network encoder is not defined for directed graphs.")
    nodes_string = create_node_string(name_dict, len(graph.nodes()))
    output = (
...
def nodes_to_text(graph, encoding_type):
    ...
    if encoding_type == "integer":
        ...
        return name_dictionaries.create_name_dict(graph, "random_integer", nnodes=1000)
