...
def check_compile(py_file, repo_name):
    command_str = f"cd {REPOS_FOLDER}/{repo_name} && python3 -m py_compile {py_file}"
...
def generate_call_graphs(repo_name):
    ...
    print(repo_name)
    os.chdir(f'{REPOS_FOLDER}/{repo_name}')

    # Run the python3 command
    command_str = f"~/miniforge3/envs/se/bin/python -m pycg  {' '.join(py_files)} --max-iter 3 -o {REPOS_ROOT}/repos_call_graphs/{repo_name}.json"
...
def main():
    ...
    created_names = [name.removesuffix(".json") for name in os.listdir(f"{REPOS_ROOT}/repos_call_graphs")]
    non_empty_created_names = []
    for created_name in created_names:
        with open(f"{REPOS_ROOT}/repos_call_graphs/{created_name}.json", "r") as f:
            data = json.load(f)
        if len(data) != 0:
